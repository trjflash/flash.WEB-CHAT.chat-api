...
class WawhController extends Controller{
    ...
    public function actionWh(){
		...
		if(Yii::$app->request->isPost){
			...
			$res = file_put_contents('/web/sites/chat.onclinic.kz/www/controllers/data.txt', file_get_contents('php://input'));
				...
				for($i = 0; $i < count($data->messages); $i++){
					...
					if(!empty(ChatsInfo::checkChat($chatId))){
						...
						try {
							...
							file_put_contents('/web/sites/chat.onclinic.kz/www/controllers/data.txt','error'.$e->getMessage());
						}
					}
					else{
